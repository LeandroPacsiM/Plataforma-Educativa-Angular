...
import { BackendCourse, BackendQuiz } from '../../../../core/models/api.models';
...
@Component({
  ...
  styles: [`
    ...
    .item-options { font-size: 0.8125rem; color: var(--fg-subtle); }
    ...
    <div class="page">
      <div class="container">
        ...
        <div class="top-bar">
          ...
          <button class="btn btn-primary" (click)="goTo('/teacher/courses/' + courseId + '/quizzes/new')">
        ...
        @if (loading()) {
          ...
          <div class="empty-state">
            ...
            <p>Crea el primer cuestionario de este curso.</p>
          </div>
        } @else {
          <div class="card">
            @for (quiz of quizzes(); track quiz.id) {
              <div class="list-item">
                <div class="item-info">
                  <div class="item-title">{{ quiz.question }}</div>
                  <div class="item-options">
                    A: {{ quiz.optionA }} &middot; B: {{ quiz.optionB }} &middot; C: {{ quiz.optionC }}
                    &middot; Correcta: <strong>{{ quiz.correctAnswer }}</strong>
                  </div>
                </div>
                <div class="item-actions">
                  <button class="btn btn-outline btn-sm" (click)="goTo('/teacher/courses/' + courseId + '/quizzes/' + quiz.id + '/edit')">Editar</button>
                  <button class="btn btn-danger btn-sm" (click)="confirmDelete(quiz)">Eliminar</button>
                </div>
              </div>
            }
          </div>
        ...
        @if (deleteTarget()) {
          <div class="confirm-overlay" (click)="deleteTarget.set(null)">
            <div class="confirm-card" (click)="$event.stopPropagation()">
              ...
              <p>¿Est&aacute;s seguro de eliminar este cuestionario?</p>
...
export class TeacherQuizzesManagerComponent implements OnInit {
  ...
  errorMsg = signal('');
  courseTitle = signal('');
  quizzes = signal<BackendQuiz[]>([]);
  deleteTarget = signal<BackendQuiz | null>(null);
  ...
  private loadQuizzes(): void {
    ...
    this.http.get<BackendCourse>(`/api/courses/${this.courseId}`).subscribe({
      next: course => {
        ...
        this.quizzes.set(course.quizzes || []);
        this.loading.set(false);
      },
      error: err => {
        this.loading.set(false);
        this.errorMsg.set(err.error?.message || 'Error al cargar los cuestionarios');
      },
    });
  }

  confirmDelete(quiz: BackendQuiz): void {
  ...
  handleDelete(): void {
    ...
    if (!target) return;
    this.http.delete(`/api/quizzes/${target.id}`).subscribe({
      next: () => {
      ...
      error: err => {
        this.deleteTarget.set(null);
        this.errorMsg.set(err.error?.message || 'Error al eliminar el cuestionario');
      },
    });
  }
